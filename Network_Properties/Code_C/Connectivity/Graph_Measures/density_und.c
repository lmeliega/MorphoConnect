...
//Description: Calcula la densidad de un grafo no dirigido representado por 
//             una matriz de adyacencia (matriz cuadrada).
//             La densidad de un grafo no dirigido es el numero de aristas 
//             o conexiones que tiene dividido entre el total de conexiones
//             que puede tener (n^2 - n) / 2. 
//Parameters:
//  In:
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    //simetric or triangular matrix (triangular matrix not yet...)
    if (!issimetricc(ma, n))
        mexErrMsgTxt("The matrix must be a simetric");    
    ...
    if(mxIsSparse(prhs[0]))
        ...
        number of columns + 1 */
        den = (double)mxGetJc(prhs[0])[mxGetN(prhs[0])] / (double)((n * n - n) / 2);
