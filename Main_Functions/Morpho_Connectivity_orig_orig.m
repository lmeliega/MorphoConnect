...
if IsMakeBoostraping
    ...
    if Nm==2
        ...
        if LoadBootstrapSamples
            ...
            [ConnectMatrix1_boot,XX,ind1]= Morpho_bootstrap01(Y1,'',0,0,ind1,Connectivity_Method); % Case of one dataset.
            ind2 = load(deblank(InputBootstrapFile(2,:)));
            ind2 = ind2.indbootstrap;
            [ConnectMatrix2_boot,XX,ind2]= Morpho_bootstrap01(Y2,'',0,0,ind2,Connectivity_Method);  % Case of one dataset.
        else
            [ConnectMatrix1,XX,indbootstrap1] = Morpho_bootstrap01(Y1,'',Nboot,Nsubj(1),'',Connectivity_Method,isReplacement); %#ok % Case of one dataset.
            [ConnectMatrix2,XX,indbootstrap2] = Morpho_bootstrap01(Y2,'',Nboot,Nsubj(2),'',Connectivity_Method,isReplacement); %#ok % Case of one dataset.
        end;
        [ConnectMatrix1_orig,ConMat1p_orig] = ConnectMatrixCalc(Y1, Connectivity_Method);
        [ConnectMatrix2_orig,ConMat2p_orig] = ConnectMatrixCalc(Y2, Connectivity_Method);
    else
        h = findobj('Tag', 'EditNSubjects'); 
        ...
        if LoadBootstrapSamples
            ...
            [ConnectMatrix1,XX,ind,ConMat1p]= Morpho_bootstrap01(Y1,'',0,0,ind,Connectivity_Method); % Case of one dataset.
        else
            [ConnectMatrix1,XX,indbootstrap,ConMat1p] = Morpho_bootstrap01(Y1,'',Nboot,Nsubj,'',Connectivity_Method,isReplacement); % Case of one dataset.
        end;
        [ConnectMatrix1_orig,ConMat1p_orig] = ConnectMatrixCalc(Y1, Connectivity_Method);
    end;
else
