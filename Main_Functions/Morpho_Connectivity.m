...
% Last Modified by GUIDE v2.5 23-Mar-2017 16:30:16
...
% --- Executes on button press in Checkbox_Dependent_Samples.
function Checkbox_Dependent_Samples_Callback(hObject, eventdata, handles)
% hObject    handle to Checkbox_Dependent_Samples (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of Checkbox_Dependent_Samples
...
if IsMeanEffectIncluded
    GrayMatVolume1 = mean(Y1,2);%sum(Y1,2);
...
if IsMakeBoostraping
    ...
    h = findobj('Tag','Checkbox_Dependent_Samples');
    isDependent = logical(get(h,'Value'));
    ...
    if Nm==2
        ...
        % When two matrices, permutation is done auntomatically ...
        [ConnectMatrix1_boot,ConnectMatrix2_boot] = Morpho_bootstrap01(Y1,Y2,Nboot,'','',Connectivity_Method,isReplacement,isDependent); % % Case of one dataset.
    else
        h = findobj('Tag', 'EditNSubjects');
        ...
        if LoadBootstrapSamples
            ...
            [ConnectMatrix1_boot,~,ind,ConMat1p_boot,ConnectMatrix1_jack]= Morpho_bootstrap01(Y1,'',0,0,ind,Connectivity_Method); % Case of one dataset.
        else
            [ConnectMatrix1_boot,~,indbootstrap,ConMat1p_boot,ConnectMatrix1_jack] = Morpho_bootstrap01(Y1,'',Nboot,Nsubj,'',Connectivity_Method,isReplacement,isDependent); % Case of one dataset.
        end;
    end;
else
    [ConnectMatrix1,ConMat1p] = ConnectMatrixCalc(Y1, Connectivity_Method);
...
% Saving Connectivity matrices and its respective info ...
ConMatsdate = datestr(clock, 'yyyy-mm-dd HH:MM:SS');
thres = 0.05;
if Nm==2  % Case of two dataset.   
    % Saving 1st group
    ConnectMatrixG1.ConMat = ConnectMatrix1;
    ConnectMatrixG1.Structures = Structures;
    ConnectMatrixG1.GLMModel = GLMModel;
    if ~isempty(OtherVariablesFile)
        [~,OtherVariablesFileName]=fileparts(OtherVariablesFile(1,:));
        ConnectMatrixG1.OtherVarsGLMModel = OtherVariablesFileName;
    end;
    ConnectMatrixG1.thres = thres;
    ConnectMatrixG1.Connectivity_Method = Connectivity_Method_Label;
    ConnectMatrixG1.NSubjects = size(Y1,1);
    ConnectMatrixG1.pValues = ConMat1p;
    Y_corrected = Y1; %#ok
    ConnectMatrixG1.Type = 'No resampled Connectivity Matrix';
    ConnectMatrixG1.date=ConMatsdate;
    ConnectMatrix=ConnectMatrixG1; %#ok
    ...
    % Saving 2nd group
    ConnectMatrixG2.ConMat = ConnectMatrix2;
    ConnectMatrixG2.Structures = Structures;
    if ~isempty(OtherVariablesFile)
        [~,OtherVariablesFileName]=fileparts(OtherVariablesFile(2,:));
        ConnectMatrixG2.OtherVarsGLMModel = OtherVariablesFileName;
    end;    
    ConnectMatrixG2.GLMModel = GLMModel;
    ConnectMatrixG2.thres = thres;
    ConnectMatrixG2.Connectivity_Method = Connectivity_Method_Label;
    ConnectMatrixG2.NSubjects = size(Y2,1);
    ConnectMatrixG2.pValues = ConMat2p;
    Y_corrected = Y2; %#ok
    ConnectMatrixG2.Type = 'No resampled Connectivity Matrix';
    ConnectMatrixG2.date=ConMatsdate;
    ConnectMatrix=ConnectMatrixG2;
    ...
    if IsMakeBoostraping
        if isDependent
            DependenceLabel='Dependent';
        else
            DependenceLabel='Independent';
        end;
        % Saving Boostrap 1st group
        ConnectMatrixG1.ConMatboot = ConnectMatrix1_boot;
        ConnectMatrixG1.Nboot = Nboot;
        ConnectMatrixG1.ResamplingType = 'Permutations';
        ConnectMatrixG1.GroupsDependence = DependenceLabel;
        ConnectMatrixG1.Type = 'Original and Resampled Connectivity Matrices with permutations';
        ConnectMatrix=ConnectMatrixG1;
        MorphoConnectMatrixFile1 = [pth,filesep,'Connect_Matrix_',name_in1,'_permutations.mat'];
        Y_corrected = Y1; %#ok
        save(MorphoConnectMatrixFile1,'ConnectMatrix','Y_corrected');

        % Saving Boostrap 2nd group
        ConnectMatrixG2.ConMatboot = ConnectMatrix2_boot;
        ConnectMatrixG2.Nboot = Nboot;
        ConnectMatrixG2.ResamplingType = 'Permutations';
        ConnectMatrixG2.GroupsDependence = DependenceLabel;
        ConnectMatrixG2.Type = 'Original and Resampled Connectivity Matrices with permutations';
        ConnectMatrix=ConnectMatrixG2;
        MorphoConnectMatrixFile2 = [pth,filesep,'Connect_Matrix_',name_in2,'_permutations.mat'];
        Y_corrected = Y2; %#ok
        save(MorphoConnectMatrixFile2,'ConnectMatrix','Y_corrected');
     end;
else  % Case of one dataset.
    ...
    if IsMakeBoostraping
        if isReplacement
            ReplacementLabel='Sampling_with_Replacement';
        else
            ReplacementLabel='Sampling_without_Replacement';
        end;
        if LoadBootstrapSamples
        ...
        ConnectMatrix.ResamplingType = 'Bootstrap';
        ConnectMatrix.SamplesReplacement = ReplacementLabel;
        ConnectMatrix.Type = 'Original, jackknifed and bootstrapped Connectivity matrices';
        ConnectMatrix.pValues_boot = ConMat1p_boot;
    else
        ConnectMatrix.Type = 'No resampled Connectivity Matrix';
    end;
    ConnectMatrix.Structures = Structures;
    ConnectMatrix.GLMModel = GLMModel;
    if ~isempty(OtherVariablesFile)
        [~,OtherVariablesFileName]=fileparts(OtherVariablesFile(1,:));
        ConnectMatrix.OtherVarsGLMModel = OtherVariablesFileName;
    end;
    ...
    ConnectMatrix.date=ConMatsdate;
    Y_corrected = Y1; %#ok
    [pth,name,ext] = fileparts(MorphoConnectMatrixFile); %#ok
    [~,name_in1] = fileparts(StructuresVolumeFile(1,:));
    if IsMakeBoostraping
        ...
        MorphoConnectMatrixFile1 = [pth,filesep,'Connect_Matrix_',name_in1,'_non_bootstrap.mat'];
    end;
    save(MorphoConnectMatrixFile1,'ConnectMatrix','Y_corrected');
    h = findobj('Tag','RadiobuttonSaveBootstrapSamples');
    SaveBootstrapSamples = get(h,'Value');
    if SaveBootstrapSamples
        [pth,name,ext] = fileparts(MorphoConnectMatrixFile1);
