function Vols = Vols_with_SPAMs(GMSegFile, WMSegFile, CSSegFile, AtlasFiles, SNFiles, Out_dir, Thresh)
%
% Syntax : 
%
%  Vols =   Vols_with_SPAMs(GMSegFile, WMSegFile, CSSegFile, AtlasFiles, SNFiles, Out_dir, Thresh)
%
...
%  SNFiles        : Normalization Transform File.
...
% Output Parameter:
%   Vols   : Volume of the Gray Matter structures weighted by the information of SPAM maps.
%
% Related References:
...
global defaults
NoExistThreshold = isempty(Thresh);
VSeg =  spm_vol(GMSegFile);
Ns = size(GMSegFile,1); % Number of subjects
if NoExistThreshold
...
Vatlas = spm_vol(AtlasFiles); 

NAtlases = size(AtlasFiles,1); % Number of SPAM maps.

Vols = zeros(Ns,NAtlases);
...
for i = 1:Ns
    ...
    %%%%%%%%%%%-----------------Thresholding-------------------%%%%%%%%%%%%%%%%
    for j=1:NAtlases
        [AtlasPath,AtlasName] = fileparts(Vatlas(j).fname); 
        disp(['Processing SPAM :   ' AtlasName]);
        ...
        Vols(i,j) = Get_Vol(VSeg(i),AtlasFiles(j,:),SNFiles(i,:),Iseg_mask, Iatlas); % Structure Volume calculation.       
    end; % end for j=1:NAtlases
end; % end for i = 1:Ns
Vols = Vols./1000; % Tranforming to cm^3 or cc.
toc;
close(H);
%========================End of main program==============================%
return;
...
%=====================Internal functions==================================%
function V = Get_Vol(VSeg, AtlasFile, Matname, Iseg_mask, Iatlas)
%
%
% Input Parameters:
...
voxsize = sqrt(sum(Vatlas.mat(1:3,1:3).^2));
voxvol = prod(voxsize);
...
V = sum(IA(:))*voxvol; % Volume calculation. 
